...
3. 見出し写真は `src/lib/plant-photos.ts` の `GUIDE_PHOTOS` に slug をキーに足す。
   **写真は記事ごとに重複させない**（2026-08-22に「タイムラインで画像を見ても記事の見分けが
   つかない」と指摘済み）。ヒーロー写真＋植物8種＝9枚は9本で使い切っているので、
   **10本目以降はPexelsから新規素材を1枚調達する**（既存と同じ「明るい・背景がすっきり」の
   基準で選び、`CREDITS.md` に出典を1行足す）
4. OGP画像 `public/og/guide-<slug>.jpg` を作る（手順は `public/og/README.md`）。
   `.astro` 側で `ogImagePath` に渡さないとトップページ用の画像のままになる
5. `npm test` を通す（`validateGuides`が形式と内部リンク先の実在を確認する）。
   **`guides.test.ts` の「GUIDES は決定したN本を過不足なく持つ」の期待値も直す**
6. 一覧ページ・トップページ・記事末尾の「ほかのガイド記事」は `GUIDES` から自動で増えるが、
   **`src/pages/guides/index.astro` の `description` とリード文だけはガイド名を手書きで
   列挙している**ので、そこへの追記を忘れないこと（過去に漏らしたことがある）
